...
import { Gamepad2, Database, Users, LogOut, BarChart3, ChevronLeft, ChevronRight, BookOpen } from 'lucide-react';
  ...
  return (
    <aside className={`relative bg-[#0a0a0a] border-r border-white/5 flex flex-col min-h-screen transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <nav className="flex-1 px-4 space-y-2 mt-4 overflow-hidden">
        ...
        {/* ✅ ลิงก์หน้า Vocabulary — เห็นได้ทุก Role */}
        <Link to="/vocabulary" title="Vocabulary" className={`flex items-center gap-3 py-3 rounded-xl transition-all ${isActive('/vocabulary') ? 'bg-indigo-600/20 text-indigo-400' : 'text-slate-500 hover:bg-white/5 hover:text-white'} ${isCollapsed ? 'justify-center px-0' : 'px-4'}`}>
          <BookOpen className="w-5 h-5 min-w-[20px]" />
          {!isCollapsed && <span className="font-bold tracking-wide text-sm whitespace-nowrap">Vocabulary</span>}
        </Link>
